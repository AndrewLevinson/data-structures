...
app.get("/ss", function(req, res) {
  ...
  // SQL query
  var q = `SELECT EXTRACT(DAY FROM sensortime) as sensorday, EXTRACT(MINUTE FROM sensortime) as sensorminute, EXTRACT(SECOND FROM sensortime) as sensorsecond, sensortime as exacttime, xValue, yValue, zValue
             FROM sensordata
             WHERE EXTRACT(DAY from sensortime) = 16
             GROUP BY sensorday, sensorminute, sensorsecond, exacttime, xValue, yValue, zValue
             ORDER by exacttime;`;
...
// respond to requests for /aameetings this query is NOT used in the app.  See /aa for live query
app.get("/aameetings", function(req, res) {
  // Connect to the AWS RDS Postgres database
  const client = new Pool(db_credentials);

  // SQL query
  var thisQuery = `SELECT addresses.street as address, addresses.zipcode as zip, addresses.lat as lat, addresses.long as long, addresses.addresspk as addressUnique, locations.name as location, locations.details as details, specialinterest, groups.name as groupname, wheelchairaccess as wheelchair, json_agg(json_build_object('day', lower(day), 'starttime', starttime, 'endtime', endtime)) as meetings
FROM times
...
GROUP BY addressUnique, address, zip, location, groupname, specialinterest, details, wheelchair
...
console.log(data);
    ...
    function displayPosts() {
        ...
        title.innerHTML = \`<span class="topic">\${filteredData[x].tag.S}</span>\`;
        ...
        rating.innerHTML = \`Rating: \${filteredData[x].rating.N} out of 10\` 
        }
    } 
    ...
    function updateQuery(e) {
        ...
        filteredData = data.filter(d => d.tag.S == currentValue); 
        }
        displayPosts()
    } updateQuery();
...
app.get("/ss", function(req, res) {
  ...
  // SQL query
  var q = `SELECT EXTRACT(DAY FROM sensortime) as recordingday, EXTRACT(SECOND FROM sensortime) as sensorsecond, xValue, yValue, zValue
