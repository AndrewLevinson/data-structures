...
async function processFiles(paths) {
        ...
        dateArr.forEach((dateElem, dateIndex) => {
          ...
          if (isDay != null && isDay != undefined && isDay != "") {
            ...
            const date = {
              ...
              meetingType: dateElem.match(
                /(?<=Meeting Type\<\/b\> )[A-Z]{1,2}/g
              ),
              specialInterest: dateElem.match(
                /(?<=Special Interest\<\/b\> )(.*)/g
              )
            };
            ...
            for (var i = 0; i < locationTable.length; i++) {
              if (
                locationTable[i].details.join() ==
                getDetails("location", firstTD).join()
              ) {
                equalDetails = true;
  ...
  writeFile(
    "data/parsed/parsed-nested-1level.json",
    JSON.stringify(meetings, null, 2)
  );
  writeFile(
    "data/parsed/parsed-meetingTable.json",
    JSON.stringify(meetingTable, null, 2)
  );
  writeFile(
    "data/parsed/parsed-groupTable.json",
    JSON.stringify(groupTable, null, 2)
  );
  writeFile(
    "data/parsed/parsed-addressTable.json",
    JSON.stringify(addressTable, null, 2)
  );
  writeFile(
    "data/parsed/parsed-locationTable.json",
    JSON.stringify(locationTable, null, 2)
  );
  // console.log('Done!');
}
...
function getDetails(selectedDetails, firstTD) {
  ...
  myCleanTD.forEach((e, i) => {
    e = e
      .replace(regFriFri, "Friday, Friday")
      .trim()
      .replace(" ,", ", ")
      .replace(/,$/, "")
      .replace(/^,/, "")
